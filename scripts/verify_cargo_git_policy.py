...
def validate_lockfiles(root: Path) -> None:
    ...
    for relative_path in LOCKFILES:
        ...
        for package in document.get("package", []):
            ...
            # workspace crates. If one reaches the shared engine transitively,
            # Cargo must record the same exact package and revision there too.
            # Only the root lock is required to contain all reviewed packages.
