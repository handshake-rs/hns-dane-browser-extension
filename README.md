...
- Adds an experimental HNS P2P recursive-DNS transport after local proof validation and authoritative DNS attempts. The relay is opt-in, and relayed answers remain untrusted input to local DNSSEC, HTTPS/SVCB, TLSA, and DANE validation. The runtime prohibits public recursive HNS DoH and normalizes historical third-party HNS DoH settings to disabled without treating them as P2P-relay consent. Manual relay peers are accepted only as IP-literal `IPv4:port` or `[IPv6]:port` endpoints and persisted only after a live HSD handshake confirms the current relay capability. The companion `hsd` responder remains an explicit operator opt-in.
